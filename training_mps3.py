...
class UNetModel(torch.nn.Module):
    ...
    def __init__(self, in_features=1, init_features=32):
        ...
        self.out_layer = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=init_features, out_channels=1, kernel_size=1, padding=0, stride=1),
        )
...
def train():
    if __name__ == '__main__':
        ...
        train_loader = DataLoader(
            dataloader, batch_size=1, shuffle=True, collate_fn=collate_fn)
        ...
        for epoch in range(num_epochs):
            ...
            for batch in train_loader:
                ...
                target_labels = target_labels.view(-1).float()
...
def test(unet):
    ...
    for f in fileNames:
        ...
        with torch.no_grad():
            probs = unet(x_input).squeeze(1)  # 扁平化输出
            probs = torch.sigmoid(probs)  # 应用 sigmoid 转换为概率
...
def testPt(unet, model_path, root_dir):
    device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    ...
    for f in fileNames:
        ...
        if image is not None:
            ...
            x_input = torch.from_numpy(image[np.newaxis, np.newaxis, ...]).float().to(device)
            assert x_input.dim() == 4, "Input tensor must have four dimensions"

            with torch.no_grad():
                probs = unet(x_input)  
                probs = torch.sigmoid(probs)  
                
            predic_ = (probs > 0.5).cpu().numpy().astype(np.uint8)[0, 0]




            result = cv.resize(predic_, (image.shape[1], image.shape[0]))
...
unet = UNetModel()
testPt(unet, model_path='unet_road_model.pt', root_dir='CrackForest-dataset-master/test/')
