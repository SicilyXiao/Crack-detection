...
class UNetModel(torch.nn.Module):
    ...
    def __init__(self, in_features=1, out_features=2, init_features=32):
        ...
        self.out_layer = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=features, out_channels=1, kernel_size=1, padding=0, stride=1),
        )
    ...
    if __name__ == '__main__':
        ...
        for epoch in range(num_epochs):
            ...
            for batch in train_loader:
                ...
                loss = criterion(m_label_out_.view(-1), target_labels.view(-1))
                loss.to(device)
                print(loss)
    ...
    for f in fileNames:
        if f.endswith(('.png', '.jpg', '.jpeg')): 
            ...
            if image is not None:  
                ...
                x_input = torch.from_numpy(image).view(1, 1, *image.shape)
                x_input = x_input.to(device)

                with torch.no_grad():  
                    probs = unet(x_input)  
                    probs = torch.sigmoid(probs)  
                    predic_ = (probs > 0.5).cpu().detach().numpy().astype(np.uint8)  # 二值化处理

                result = cv.resize(predic_[0], (image.shape[1], image.shape[0]))
                cv.imshow("unet-segmentation-demo", result)
                cv.waitKey(0)
