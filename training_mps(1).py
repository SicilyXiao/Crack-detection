...
def testPt(unet, model_path, root_dir):
    ...
    unet.eval()  # 将模型设置为评估模式
    unet.to(device)  # 将模型移动到设备上
    ...
    for f in fileNames:
        if f.endswith(('.png', '.jpg', '.jpeg')):
            ...
            if image is not None:
                ...
                # 准备输入数据，确保是4D张量 [1, 1, height, width]
                image = image[np.newaxis, np.newaxis, :, :]
                x_input = torch.tensor(image, dtype=torch.float32).to(device)
                # 使用 no_grad 来禁用梯度计算
                with torch.no_grad():
                    probs = unet(x_input)  # 获取模型输出
                    probs = torch.sigmoid(probs) # 应用 sigmoid 函数

                # 处理预测结果并显示
                #probs = probs.squeeze(0)  # 移除批次维度，形状变为 [2, height, width]
                #probs = probs[1]  # 选择第二个通道，形状变为 [height, width]
                predic_ = (probs > 0.5).cpu()
                predic_ = predic_.squeeze(0)
                predic_ = predic_[1]

                predic_ = predic_.numpy().astype(np.uint8)

                #predic__resized = cv.resize(predic_, (w, h))
                result = cv.resize(np.uint8(predic_), (w, h))
...
unet = UNetModel()
testPt(unet, 'unet_road_model.pt', 'CrackForest-dataset-master/test/')
